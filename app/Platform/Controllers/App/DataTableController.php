...
//      echo json_encode($records);
        ...
        if(strpos($url, 'Customer') or strpos($url, 'customers')){
            for ($i=0;$i<count($records);$i++){
                $campaign_text = $records[$i]['campaign_text'];
                $customer_number = $records[$i]['number'];
                $customer_number = str_replace("-", "", $customer_number);


                $mobile = DB::table('customers')->where([['email', '=', $records[$i]['email']], ['customer_number', '=', $customer_number]])->select('mobile')->orderBy('id','asc')->get();
                $expiration = DB::table('customers')->where([['email', '=', $records[$i]['email']], ['customer_number', '=', $customer_number]])->select('expiration')->orderBy('id','asc')->get();
