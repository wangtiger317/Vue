...
class DataFormController extends \App\Http\Controllers\Controller
{
    ...
    */
    ...
    public function getDataForm(Request $request) {
      ...
      if($uuid !== null){
          if(strpos($model, 'Customer') !== false)
          {
    ...
    public function postSaveRecord(Request $request) {
      ...
      if(strstr($model, 'Customer')) {
          ...
          $customer = Customer::whereUuid($uuid)->first();
      ...
      if(strstr($model, 'Customer')){
          if($email != '"'.$customer->email.'"'){
              $v = Validator::make($request->formModel, [
                  'email' => ['required', 'email', 'max:64', Rule::unique('customers')->where(function ($query) use ($campaign_id) {
...
//            if($value['value'] == 0) $query->{$column} = 0;
            $query->{$column} = ($value['value'] == 0)? 0:$value['value'];
