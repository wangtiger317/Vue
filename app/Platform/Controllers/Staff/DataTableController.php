...
//                echo auth()->user()->role;
            ...
            if ($filter['type'] == 'relation') {
                ...
                if ($relation['type'] == 'hasOne' || $relation['type'] == 'belongsTo') {
                    if ($permission == 'all') {
                        ...
                        $items = $model::with($relation['with'])->getRelation($relation['with'])->selectRaw('' . $relation['pk'] . ' as pk, ' . $relation['val'] . ' as val')->where(function($query) use($where) { if ($where != '') return $query->whereRaw($where); })->where('id', $campaign->id)->orderBy($relation['orderBy'], $relation['order'])->get();
                    }
                }
                ...
                if ($relation['type'] == 'belongsToMany') {
                    if ($permission == 'all') {
                        ...
                        $items = DB::table($relation['table'])->selectRaw('' . $relation['pk'] . ' as pk, ' . $relation['val'] . ' as val')->where(function($query) use($where) { if ($where != '') return $query->whereRaw($where); })->where('id', $campaign->id)->orderBy($relation['orderBy'], $relation['order'])->get();
        ...
        if ($permissions['view'] == 'all') {
            if($search !== null && strlen($search) > 1) {
                // Search query
                $search_query = $query->withoutGlobalScopes()->where('campaign_id', $campaign->id)->where(function ($query) use($search, $searchColumns) {
                ...
                $total = $total->withoutGlobalScopes()->where('campaign_id', $campaign->id)->count();

                // Get paginated result
                $records = $search_query->withoutGlobalScopes()->where('campaign_id', $campaign->id)->with($withRelations);
            } else {
                // Get total
                $total = clone $query;
                $total = $total->withoutGlobalScopes()->where('campaign_id', $campaign->id)->count();
                ...
                // Get paginated result
                $records = $query->withoutGlobalScopes()->where('campaign_id', $campaign->id)->with($withRelations);
        ...
        */
        ...
        if(strpos($url, 'Customer') or strpos($url, 'customers')){
            for ($i=0;$i<count($records);$i++){
                $mobile = DB::table('customers')->where([['email', '=', $records[$i]['email']], ['campaign_id', '=', $campaign->id]])->select('mobile')->orderBy('id','asc')->get();
                $expiration = DB::table('customers')->where([['email', '=', $records[$i]['email']], ['campaign_id', '=', $campaign->id]])->select('expiration')->orderBy('id','asc')->get();
                $locale = DB::table('customers')->where([['email', '=', $records[$i]['email']], ['campaign_id', '=', $campaign->id]])->select('locale')->orderBy('id','asc')->get();

                $records[$i]['phone'] = $mobile[0]->mobile;
                $records[$i]['locale'] = $locale[0]->locale;
                $records[$i]['expiration'] = $expiration[0]->expiration;
//                $records[$i] = (object)array_merge( (array) $records[$i], array( '0'=>$mobile[0]->mobile, 'name' => $records[$i]['name'], 'email' => $records[$i]['email'], 'updated_at' => $records[$i]['updated_at'], 'active' => $records[$i]['active'], 'uuid' => $records[$i]['uuid'], 'avatar' => $records[$i]['avatar'], 'points' => $records[$i]['points'], 'campaign_text' => $records[$i]['campaign_text'], 'number' => $records[$i]['number'], 'shelf_life' => $shelf_life[0]->shelf_life ) );
            }
        }

        if(strstr($model, 'Customer')){

            $shelfLife = $model::getShelfLife()[auth()->user()->role];

            return response()->json(['status' => 'success', 'settings' => $settings, 'headers' => $headers, 'total' => $total, 'records' => $records, 'actions' => $actions, 'shelfLife' => $shelfLife, 'showExport' => $showExport, 'filters' => $filters, 'translations' => $translations, 'filteredResults' => $filteredResults], 200);
...
//        return response()->json(['status' => 'success', 'settings' => $settings, 'headers' => $headers, 'total' => $total, 'records' => $records, 'actions' => $actions, 'showExport' => $showExport, 'filters' => $filters, 'translations' => $translations, 'filteredResults' => $filteredResults], 200);
    ...
    /**
     * Save one expiration date.
     *
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function postSaveExpiration(Request $request){
        $model = $request->model;
        $uuid = $request->uuid;
...
//            echo $query;
    ...
    /**
     * Delete one expiration date.
     *
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function postDeleteExpiration(Request $request){
      $model = $request->model;
      $uuid = $request->uuid;
