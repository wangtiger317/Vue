...
class PointController extends Controller {
    ...
    public function postGetClaimToken(Request $request) {
      ...
      $last_token = Code::where('type', 'token')->where('customer_id', auth('customer')->user()->id)->select('code')->orderBy('id','asc')->first();
...
//      $code->code = $token;
    ...
    public function postGetEarnOrRedeem(Request $request){
        $account = app()->make('account');
        $campaign = \Platform\Models\Campaign::withoutGlobalScopes()->whereUuid(request('campaign', 0))->firstOrFail();
        return response()->json([
            'status' => 'success',
            'earn_or_redeem' => $campaign->earn_point
        ]);
    }
    /**
    ...
    public function postVerifyCustomerCode(Request $request) {
      ...
      $history->event = 'Código introduzido pelo cliente';
    ...
    public function postProcessMerchantEntry(Request $request) {
      ...
      $history->event = 'Código introduzido pelo funcionário';
      $history->created_by = $campaign->created_by;
