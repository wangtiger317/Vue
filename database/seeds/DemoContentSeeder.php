...
class DemoContentSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if ($campaign_count > 0) {
          foreach (range(1, $campaign_count) as $index) {
            ...
            $campaign->content = [
              ...
              "homeHeaderContent" => "<p>Poderá ganhar pontos sempre que compra connosco e depois poderá trocá-los por prémios do nosso catálogo abaixo.</p>",
              "contactHeaderTitle" => "Contacte-nos",
              "cardHeaderTitle" => "Cartão",
            ...
            for ($i = 0; $i < mt_rand($customers_per_campaign - 3, $customers_per_campaign); $i++) {
              ...
              if ($campaign->signup_bonus_points > 0) {
                ...
                $history->event = 'Bónus: cliente registou-se';
        ...
        // Add history
        $history_earn_events = ['Código introduzido pelo cliente', 'Código introduzido pelo funcionário', 'Creditado por QR code'];
        $history_earn_points = [20, 50, 75, 100, 125, 150, 175, 200, 225, 250, 275, 300, 325, 350];
        $history_redeem_events = ['Atribuído por código', 'Atribuído com QR code'];
