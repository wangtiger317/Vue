<template>
  <v-app
    ...
    <v-app-bar
      ...
      <v-spacer></v-spacer>
      <v-toolbar-items>
        <v-btn class="hidden-sm-and-down" text :style="{color: website.theme.primaryTextColor}" v-for="(item, index) in topNav" v-if="!item.sideNavOnly" :key="'topNav_' + index" :href="item.href" exact>{{ item.name }}</v-btn>
      </v-toolbar-items>
    ...
    <v-card
      ...
      tile
    >
<!--      <v-card-actions class="justify-center body-2 footer-1" :style="{'background-color': website.theme.primaryColor, 'color': website.theme.primaryTextColor}">-->
<!--        &copy;{{ new Date().getFullYear() }} — {{ account.app_name }} <span class="mx-1">&bull;</span> <router-link :to="{name: 'legal'}" :style="{'color': website.theme.primaryTextColor}">Termos & Privacidade</router-link>-->
<!--        <span class="mx-1">&bull;</span>support@cartaocliente.com-->
<!--&lt;!&ndash;          <span v-html="account.app_contact"></span>&ndash;&gt;-->
<!--      </v-card-actions>-->

        <v-card class="mx-auto" min-width="320" align="center"
                        :style="{'background-color': website.theme.primaryColor, 'color': website.theme.primaryTextColor}">

            &copy;{{ new Date().getFullYear() }} — {{ account.app_name }}

            <span class="mx-1">&bull;</span>
            <router-link :to="{name: 'legal'}" :style="{'color': website.theme.primaryTextColor}">Termos & Privacidade
            </router-link>
            <span class="mx-1">&bull;</span>
            <span>support@cartaocliente.com</span>
        </v-card>
...
<!--      class="termsConsent"-->
<!--    >-->
<!--        Usamos cookies para melhorar o desempenho e experiência. Ao continuar, declara aceitar todos os cookies e os Termos & Privacidade.-->
<!--      <v-btn-->
<!--        color="white"-->
<!--        text-->
<!--        :to="{ name: 'legal' }"-->
        ...
        return [
          {
            ...
            icon: 'add_box',
            route_name: 'more',
            href: 'https://cartaocliente.com/more/'
          },
          {
            name: 'Suporte',
            icon: 'contact_support',
            route_name: 'more/#suporte',
            href: 'https://cartaocliente.com/more/#suporte'
          }
        ]
      },
      nav() {
