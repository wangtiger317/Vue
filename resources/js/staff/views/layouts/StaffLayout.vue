    ...
    <v-footer absolute app class="pa-0" height="auto" :style="{'background-color': campaign.theme.drawer.backgroundColor, 'color': campaign.theme.drawer.textColor}">
        ...
        >
          <v-card-actions class="justify-center body-2" :style="{'background-color': campaign.theme.primaryColor, 'color': campaign.theme.primaryTextColor}">
            &copy;{{ new Date().getFullYear() }} — {{ campaign.business.name }} <span class="mx-2">&bull;</span> <span :style="{'color': campaign.theme.primaryTextColor}">Área de Funcionário</span>
          </v-card-actions>
        </v-card>
    </v-footer>
    ...
    computed: {
      topNav() {
        if (this.$auth.check()) {
          return [
            {
                name: this.$t('Clientes'),
                icon: 'contacts',
                route_name: 'customers',
                to: {name: 'customers'}
            },
            {
              name: this.$t('Atividade'),
              icon: 'dashboard',
