<template>
  <v-card>
    <v-toolbar :tabs="tabCount > 1" flat v-if="!loadingForm">
      ...
      <template v-slot:extension v-if="tabCount > 1">
        <v-tabs
          slot="extension"
          v-model="selectedTab"
          color="grey darken-3"
    ...
    <div v-if="!loadingForm && limitReached">
      <v-card-text class="pt-0">
          ...
          >
              ...
              <span>Esta campanha atingiu o limite de registos. Não podem ser adicionados mais clientes. Por favor contacte-nos para que possamos averiguar.</span>
...
<!--      <v-card-actions>-->
<!--        <v-spacer></v-spacer>-->
<!--        <v-btn large text  :to="{'name': 'user.billing'}">Manage subscription</v-btn>-->
<!--      </v-card-actions>-->
    </div>
...
-->
                <div v-for="(form_item, form_index) in sub.items">
                  ...
                  <div v-if="form_item.type == 'text' || form_item.type == 'email'">
                    <v-text-field
                      ...
                      :label="(form_item.required) ? form_item.text + $t(' (obrigatório)') : form_item.text"
                  ...
                  <div v-if="form_item.type == 'date_time'">
                    ...
                    >
                      <template slot="actions" slot-scope="{ parent }">
                        <v-btn  text @click="parent.okHandler">OK</v-btn>
...
<!--              </div>-->
      ...
      <v-card-actions v-if="!loadingForm">
        <v-spacer></v-spacer>
        <v-btn  text _loading="form.loading" :disabled="form.loading" large type="submit" class="ml-0">{{ (uuid === null) ? $t('Criar') : $t('Guardar') }}</v-btn>
        <v-btn color="secondary" text :disabled="form.loading" large @click="$emit('data-list-events', {'closeDialog': true, 'reload': false})">{{ $t('Fechar') }}</v-btn>
      </v-card-actions>
    </v-form>
  ...
  export default {
    ...
    methods: {
      ...
      saveForm() {
        ...
        let settings = { headers: { 'content-type': 'multipart/form-data' ,'charset':'utf-8'} }
        ...
        formData.append('campaign_uuid', this.$store.state.app.campaign.uuid) /*Add Campaign uuid*/
        ...
        axios.post(this.api + '/save', formData, settings)
            ...
            .catch(err => {
                if (typeof err.response !== 'undefined') {
                    ...
                    if(this.formModel.error == 'limitation_reached'){
                        console.log('==Plan limitation==')
                    ...
                    if( err.response.data.errors != null){
                        ...
                        if (typeof err.response.status !== 'undefined' && err.response.data.msg !== 'undefined') {
                            if (err.response.status == 422) {

                                let action = err.response.data.errors
                                console.log(action);
                                this.$root.$snackbar(action[field][0])
                                return
        ...
        return new Promise((resolve, reject) => {
          ...
          .then(res => {
            if (res.data.status === 'success') {
              ...
              that.email = res.data.values.email
