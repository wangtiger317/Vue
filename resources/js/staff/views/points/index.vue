            ...
            <v-container fill-height>
              <v-layout row wrap align-center>
                ...
                <div class="mt-3 mb-7 pa-0">
                  <h1 class="display-1 mb-1">Creditar pontos aos clientes</h1>
                  <div>Abaixo estão as opções para creditar pontos aos clientes.</div>
                </div>
              </v-flex>
            </v-layout>
          </v-container>
      ...
      <v-card>
        ...
        <v-tabs-items v-model="customerCode.tab" :touchless="false" class="mx-2">
          <v-tab-item :value="'generate'">
            ...
            >
              <v-card-text>
                <p class="body-1">Crie um código para ser partilhado com o cliente. Este código só pode ser usado uma vez.</p>
                  ...
                  <a href="javascript:void(0);" v-if="customerCode.code !== false" @click="customerCode.code = false">Criar um novo código</a>

               </v-card-text>
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="primary" :loading="customerCode.loading" :disabled="customerCode.code !== false" type="submit">Criar</v-btn>
                <v-btn color="secondary" text @click="dialog.claim.code = false">Fechar</v-btn>
              </v-card-actions>
          ...
          <v-tab-item :value="'active'">
              ...
              <v-card-text v-if="Object.keys(activeCustomerCodes).length == 0">
                <p class="body-1">Não há códigos ativos.</p>
              </v-card-text>

              <v-card-text v-if="Object.keys(activeCustomerCodes).length > 0">
                <p class="body-1">Este códigos estão ativos e ainda não foram reclamados pelos clientes.</p>

                <v-simple-table>
                  <thead>
                    <tr>
                      <th class="text-xs-left">Código</th>
                      <th class="text-xs-right">Pontos</th>
                      <th class="text-xs-left">Expira</th>
                    </tr>
                  </thead>
                  <tbody>
    ...
    <v-dialog v-model="dialog.claim.customerNumber" persistent max-width="380">
      <v-card>
        <v-card-title class="headline">Creditar pontos</v-card-title>
          ...
          >
            <v-card-text>
              <p class="body-1" v-if="customerNumber.credited === false">Introduza o número de pontos a creditar e o número de cliente.</p>
              <p class="body-1" v-if="customerNumber.credited !== false">Pontos adicionados com sucesso.</p>
    ...
    data () {
      return {
        ...
        expires: [
          ...
          {value: 'year', text: 'Expira num ano'}
        ],
        customerCode: {
          tab: 'generate',
          loading: false,
      ...
      claimOptions () {
        return  [
          ...
          },
          {
            ...
            icon: 'textsms',
            title: 'Criar código',
            description: 'Crie um código que pode ser partilhado com o cliente.'
          },
          {
          ...
          },
          {
            ...
            description: 'Credite pontos a um cliente inserindo o seu número de cliente'
          }
        ]
      }
    }
