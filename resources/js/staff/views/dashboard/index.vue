  ...
  <v-container fluid fill-height class="px-0 pt-0 pb-5">
    <v-layout align-center justify-center row fill-height wrap>
      ...
      <v-flex xs10 sm8 md7 lg5 xl4 class="mt-5">
        <v-card-text v-if="$auth.user().expired" class="pl-0 pr-0">
          ...
          <v-tabs-items v-model="selectedTab">
            <v-tab-item :value="'history'">
              <v-card-text>
                ...
                <p class="title ma-2" v-if="loading === false && Object.keys(history).length == 0">Ainda não há atividade para esta campanha.</p>
                ...
                >
                  ...
                  <v-layout>
                    <v-flex>
                      ...
                      <strong>{{ (item.points > 0) ? '+' : '' }}{{ new Intl.NumberFormat(locale.replace('_', '-')).format(item.points) }} pontos</strong>,
