  ...
  <v-card>
    ...
    <div v-if="!loadingForm && limitReached">
      <v-card-text class="pt-0">
        ...
        <p class="subtitle-1">Atingiu o máximo de {{ max }} {{ translations.items_lowercase }}. Se quiser criar mais {{ translations.items_lowercase }}, por favor altere a sua subscrição.</p>


      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn large text :color="app.color_name" :to="{'name': 'user.billing'}">Gerir subscrição</v-btn>
      </v-card-actions>
    </div>
