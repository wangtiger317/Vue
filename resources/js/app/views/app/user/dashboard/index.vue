    ...
    <v-container fluid grid-list-xl v-if="!loading && stats.total.onboardingStep < 6">
      <v-layout row>
        <v-flex lg12 xl10>
          <v-layout row wrap>
            <v-flex xs12 lg7>
              <h1 class="main-title mb-3">Comece a sua primeira campanha de fidelização</h1>

              <v-stepper v-model="stats.total.onboardingStep" vertical>
                <v-stepper-step :complete="stats.total.onboardingStep > 1" step="1" :color="app.color_name">Criar um estabelecimento</v-stepper-step>

                <v-stepper-content step="1">
                  <v-icon :color="app.color_name" size="64" class="mb-2">business</v-icon>
                  <p class="subheading">O estabelecimento é a entidade onde irá associar Funcionários e Campanhas de Fidelização. Vamos começar criando um estabelecimento.</p>
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn :color="app.color_name" text :to="{name: 'user.businesses'}">Criar um estabelecimento <v-icon>keyboard_arrow_right</v-icon></v-btn>
                  </v-card-actions>
                </v-stepper-content>

                <v-stepper-step :complete="stats.total.onboardingStep > 2" step="2" :color="app.color_name">Criar um funcionário</v-stepper-step>

                <v-stepper-content step="2">
                  <v-icon :color="app.color_name" size="64" class="mb-2">supervised_user_circle</v-icon>
                  <p class="subheading">Um funcionário pode adicionar pontos ao cliente e atribuir prémios, descontando os pontos. O Funcionário pode estar inserido num estabelecimento ou em vários.</p>
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn :color="app.color_name" text :to="{name: 'user.staff'}">Criar um funcionário <v-icon>keyboard_arrow_right</v-icon></v-btn>
                  </v-card-actions>
                </v-stepper-content>

                <v-stepper-step :complete="stats.total.onboardingStep > 3" step="3" :color="app.color_name">Criar um prémio</v-stepper-step>

                <v-stepper-content step="3">
                  <v-icon :color="app.color_name" size="64" class="mb-2">fas fa-gift</v-icon>
                  <p class="subheading">Os seus clientes poderão trocar pontos por prémios. Uma campanha pode ter um ou mais prémios.</p>
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn :color="app.color_name" text :to="{name: 'user.rewards'}">Criar um prémio <v-icon>keyboard_arrow_right</v-icon></v-btn>
                  </v-card-actions>
                </v-stepper-content>

                <v-stepper-step :complete="stats.total.onboardingStep > 4" step="4" :color="app.color_name">Criar campanha de fidelização</v-stepper-step>

                <v-stepper-content step="4">
                  <v-icon :color="app.color_name" size="64" class="mb-2">record_voice_over</v-icon>
                  <p class="subheading">Uma campanha é um site onde os clientes podem registar-se, ganhar pontos e trocá-los por prémios.</p>
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn :color="app.color_name" text :to="{name: 'user.campaigns'}">Criar uma campanha <v-icon>keyboard_arrow_right</v-icon></v-btn>
                  </v-card-actions>
                </v-stepper-content>

                <v-stepper-step step="5" :color="app.color_name">Promova a sua campanha para que os seus clientes se registem</v-stepper-step>

                <v-stepper-content step="5">
                  <v-icon :color="app.color_name" size="64" class="mb-2">contacts</v-icon>
                  <p class="subheading">Promova o site da sua campanha para que os seus clientes possam registar-se, consultar o saldo de pontos e verem o catálogo de prémios. Pode também registar um cliente fazendo login na área de funcionário. Encontre os endereços do site e área de funcionário através do menú em Fidelização > Campanhas.</p>
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn :color="app.color_name" text :to="{name: 'user.customers'}">Ver clientes <v-icon>keyboard_arrow_right</v-icon></v-btn>
                  </v-card-actions>
                </v-stepper-content>
              </v-stepper>
            ...
            <v-flex xs12 md7>
              ...
              <v-card>
                <div class="tabs-s1">
                  ...
                  >
                    <v-tab :href="'#customers'">
                      <div class="tabs__item__title">Clientes</div>
                      ...
                      <div v-if="stats.customers.signupsChange != 0" class="tabs__item__sub-title">vs. últimos 7 dias</div>
                    </v-tab>
                    <v-tab :href="'#earning'">
                      <div class="tabs__item__title">Pontos</div>
                      ...
                      <div v-if="stats.earnings.earningsChange != 0" class="tabs__item__sub-title">vs. últimos 7 dias</div>
                    </v-tab>
                    <v-tab :href="'#spending'">
                      <div class="tabs__item__title">Prémios</div>
                      ...
                      <div v-if="stats.spendings.spendingsChange != 0" class="tabs__item__sub-title">vs. últimos 7 dias</div>
                    </v-tab>
                  </v-tabs>
                ...
                <v-card-actions class="pt-0">
                  ...
                  -->
                  <v-spacer></v-spacer>
                    <v-btn text class="white" :to="{name: 'user.customers'}" v-if="statTabs=='customers'">Clientes <v-icon dark>keyboard_arrow_right</v-icon></v-btn>
                    <v-btn text class="white" :to="{name: 'user.analytics.earning'}" v-if="statTabs=='earning'">Pontos <v-icon dark>keyboard_arrow_right</v-icon></v-btn>
                    <v-btn text class="white" :to="{name: 'user.analytics.spending'}" v-if="statTabs=='spending'">Prémios <v-icon dark>keyboard_arrow_right</v-icon></v-btn>
                  </v-card-actions>
              </v-card>
            </v-flex>
            <v-flex xs12 md5>
              <h1 class="main-title mb-3">Resumo</h1>
              <v-card :color="app.color_name" class="current--status">
                    <v-container fluid grid-list-lg>
                      <v-layout row>
                        <v-flex xs12>
                          <div class="statics">
                            <div class="body-1">Total de clientes</div>
                            <div class="lg-text">{{ formatNumber(stats.total.customers) }}</div>
                          </div>
                          <div class="mb-5">
                            <div class="d-flex body-1 custom-caption">Prémios
                              <v-spacer></v-spacer>
                              <span class="text-xs-right">{{ formatNumber(stats.total.rewards) }}</span>
                            </div>
                            <div class="d-flex body-1 custom-caption">Segmentos
                              <v-spacer></v-spacer>
                              <span class="text-xs-right">{{ formatNumber(stats.total.segments) }}</span>
                            </div>
                            <div class="d-flex body-1 custom-caption">Campanhas
                              <v-spacer></v-spacer>
                              <span class="text-xs-right">{{ formatNumber(stats.total.campaigns) }}</span>
                            </div>
                            <div class="d-flex body-1 custom-caption">Estabelecimentos
                              <v-spacer></v-spacer>
                              <span class="text-xs-right">{{ formatNumber(stats.total.businesses) }}</span>
                            </div>
                            <div class="d-flex body-1 custom-caption">Funcionários
                              <v-spacer></v-spacer>
                              <span class="text-xs-right">{{ formatNumber(stats.total.staff) }}</span>
                            </div>
                          </div>
                          <div v-if="Object.keys(stats.popularRewards).length > 0">
                            <div class="d-flex caption custom-caption">Prémios favoritos
                              <v-spacer></v-spacer>
                              <span class="text-xs-right">Resgatados pelo cliente</span>
                            </div>
                    ...
                    <v-card-actions v-if="Object.keys(stats.popularRewards).length > 0">
                      <v-spacer></v-spacer>
                      <v-btn text :color="app.text_color_name" :to="{name: 'user.rewards'}">Todos os prémios
      ...
      axios
        ...
        .then(response => {
          ...
          let customerChartData = _.map(stats.customers.signupsCurrentPeriod, (count, date) => {
            return [moment(date).format('MMM D'), count]
          });
