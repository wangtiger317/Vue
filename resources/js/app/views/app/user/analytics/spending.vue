  ...
  <div style="height: 100%">
    ...
    <v-card flat color="transparent" v-if="!loading && stats.total.onboardingStep < 6">
      <v-toolbar flat color="transparent">
        <v-toolbar-title>Prémios</v-toolbar-title>
      </v-toolbar>
      <div  class="text-xs-center">
        <div>
          <v-icon size="72" :color="$store.getters.app.color_name">fas fa-gift</v-icon>
        </div>
        <h1 class="title my-4">Ainda não há estatísticas</h1>
        <div class="mx-5 pb-4">
          <p class="subheading">As estatísticas dos prémios oferecidos irão aparecer aqui.</p>
        </div>
      </div>
    </v-card>
            ...
            let chartData = _.map(stats.chart, (o, i) => {
              if (i == 0) {
                ...
                for (let index = 0; index < Object.keys(o).length; index++) {
                  if (index == 0) {
                    o[index] = moment(o[index]).format('MMM D')
                  }
                }
                return o
              }
