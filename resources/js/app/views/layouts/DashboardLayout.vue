<template>
  <v-app
    ...
    <v-navigation-drawer
      ...
      <v-list dense nav shaped key="navKey">
    ...
    <v-app-bar
      ...
      <v-btn dark text v-if="!$auth.check()" :href="app.app_scheme + '://' + app.app_host">
        <v-icon color="white" size="15" class="mr-1">home</v-icon> Início
      </v-btn>
...
<!--          <v-toolbar-items v-if="$auth.user().role == 3">-->
<!--              <v-btn color="white" text v-for="item in topnav"-->
<!--                v-bind:todo="item"-->
<!--                :to="item.to"-->
<!--              >{{item.text}}</v-btn>-->
<!--          </v-toolbar-items>-->
        ...
        <v-list>
          ...
          <v-list-item v-if="$auth.user().role === 3" :to="{name: 'user.billing'}">
            <v-list-item-content>
              <v-list-item-title>Subscrição</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
          <v-divider :inset="false" class="grey lighten-2"></v-divider>
...
<script>
  ...
  export default {
    ...
    created: function () {
      ...
      if(this.$auth.user().expired && this.$route.path != '/dashboard'){
         this.$router.push({name: this.dashboard})
      }
