    ...
    >
      ...
      <v-menu bottom left nudge-bottom="60px" v-if="$vuetify.breakpoint.mdAndUp && $auth.check()">
        ...
        <v-list>
          ...
          <v-list-item @click="$router.push({name: 'card'})">
              <v-list-item-content>
                  <v-list-item-title>{{ $t('card') }}</v-list-item-title>
              </v-list-item-content>
          </v-list-item>
    ...
    nav() {
      ...
      nav.unshift(
        ...
        },
        {
            name: this.$t('card'),
            icon: 'account_box',
            route_name: this.$auth.check()?'card':'login',
            to: this.$auth.check()?{name: 'card'}:{name: 'login'}
        },
      )
